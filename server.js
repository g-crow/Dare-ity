...
app.post('/api/make_pledge', function(req, res){
  const {broadcaster_id, npo_id, dare_id, user_dare_id, pledger_id, pledgeAmount} = req.body;
  var queryString = "INSERT INTO pledge (broadcaster_id, npo_id, dare_id, user_dare_id, pledger_id, pledgeAmount) " + 
                    "VALUES (" + broadcaster_id + "," npo_id "," + dare_id + "," + user_dare_id + "," + pledger_id ", " + pledgeAmount ")" 
  pool.query(queryString, function(err, result) {
    if(err){
      console.error('error', err.message)
    } else {
